"""
PyramidMamba: A Mamba-based decoder head with pyramid pooling for semantic segmentation.
...
class PyramidMambaHead(BaseDecodeHead):
    def __init__(
        ...
        **kwargs,
        ...
        if "channels" not in kwargs:
            kwargs["channels"] = decoder_channels // 2
        if "in_index" not in kwargs:
            kwargs["in_index"] = [0, 1]
        super().__init__(
        ...
        if len(self.in_channels) != 2:
            raise ValueError(
                "PyramidMambaHead expects exactly 2 inputs (low/high), "
                f"but got {len(self.in_channels)}. "
                "Set backbone out_indices and decode_head in_index accordingly."
            )
        self.b3 = Block(
            in_chs=self.in_channels[-1],
        ...
        self.pre_conv = ConvBNReLU(self.in_channels[0], decoder_channels)
        self.head = nn.Sequential(
            ConvBNReLU(decoder_channels, self.channels),
            nn.Upsample(
    ...
    def forward(self, inputs):
        x0, x3 = self._transform_inputs(inputs)
        x3 = self.b3(x3)
        x3 = self.up_conv(x3)
